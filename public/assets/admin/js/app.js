  ...
  $.fn.todolist = function (options) {
    ...
    $('.fileupload').each(function () {
        $(this).fileupload({
            dropZone: $(this),
            add: function (e, data) {
                $(this).parent().find('span').html('Uploading...');
                data.submit();
            },
            done: function (e, data) {
                $('.icon-'+data.result.module).attr('src',data.result.path);
                $(this).parent().find('span').html('Upload finished.');
                $('input[name=icon_'+data.result.module+']').val(data.result.file_name);
                $('.btn-icon-remove[data-type='+data.result.module+']').removeClass('hide').addClass('show');
...
$('.btn-delete').on('click', function(){
    if ($(this).data('item-type') == 'recyclebin') {
        return confirm('Are you sure you want to permanently delete all these stories');
    }
    return confirm('Are you sure you want to delete this ' + $(this).data('item-type') + '?');
});

$(document).on('change','#achievement_category',function(){
    $.post('/admin/achievements/ajax-subcategories',{id:$(this).val()},function(response){
        $('#achievement_sub_category').html('<option value=""></option>')
        $.each(response,function(key, value) {
            $('#achievement_sub_category').append('<option value="'+value.id+'">'+value.title+'</option>');
        });
    },'JSON');
});
